...
6. In `control.r`, run these blocks from top down as needed: single run-clicks here let you read in csvs (do minimally as ~1/6 min read-in per scenario), build `data.frame`s, and generate lists and plots, individually with individual functions or in batches with pre-defined templates.
