...
6. In `control.r`, run these blocks from top down as needed: single run-clicks here let you read in csvs, build `data.frame`s, and generate table and plots. Choice here to make individual or batch plots using pre-defined templates, or see/export tables as `tibble`/`.csv` 
