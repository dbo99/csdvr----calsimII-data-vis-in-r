...
[calsim](https://water.ca.gov/Library/Modeling-and-Analysis/Central-Valley-models-and-tools/CalSim-2) is a water resources planning model for a large part of california, developed jointly by [usbr](https://www.usbr.gov/) and [ca-dwr](https://water.ca.gov/) to help analyze the limitations and possibilities of, and benefits and impacts from, the interconnected [central valley project](https://www.usbr.gov/mp/cvp/) and [state water project](https://water.ca.gov/Programs/State-Water-Project), under alternative infrastructure, regulatory, operational, or climate scenarios.   calsim also helps address related engineering and scientific questions, with its output used as input by popular biologic, economic, groundwater, and hydrodynamic models

   key file  | use
------------ | -------------
[`csv_list.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/csv_list.r) | list csv file names and assign brief unique scenario nicknames
[`fun_defs.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/fun_defs.r) | function definitions - add to here as necessary
[`scen_facts.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/scenfacts.r) | list same nicknames top down to order the left to right position of scenarios in the plot (instead of alphabetic)
[`plotexport.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plotexport.r) | select which plot scripts to run for export (wait 1-2 minutes for a comprehensive set or a few seconds for 20 or so?)
[`control.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/control.R) | main user interface - actually read-in data and analyze or batch export plots
...
### pb - plot bars - annual mean  ###
   function  | for
------------ | -------------
[pb_mn_ann_perav_taf(df)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/june2018plots/c406%26del_cvp_total_n%26del_cvp_total_s%26del_swp_tot_n%26del_swp_tot_s_Outflow%26DelivBars.jpg) |   bars of mean annuals with vertical labels (good for ~10 scenarios?)
...
[pb_mn_ann_perav_taf_d_nolab_rank(df_diff)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/june2018plots/do%26cvpswp_n%26sdelivs_plot_bardiffs.jpg) | label-less difference bars of mean annuals without labels, ranked highest to lowest left to right (good for very many scenarios)
...
### monthly - line ### 
   function  | for
------------ | -------------
[p_mon_excd_taf(df)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/june2018plots/s4%26s1_storagemonthly.jpg) |   monthly exceedance in taf style 1 (dvs plotted on single plot, with linetype differences (eg solid vs. dash)
[p_mon_excd2_taf(df)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/july2018plots/s1%26s4%26s6%26s8%26s11_storagemonthly.jpg) |  monthly exceedance in taf, style 2: plot ("facet") for each dv in a row
...
[pdr3_ann_perav_wysum_taf_d(df_diff)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/june2018plots/del_swp_tot_s_annsumdenridgediff3.jpg) | style 3, difference, color gradient for water year total
...
## timeseries ridges - pr - plot ridges - one dv only! ##
   function  | for
------------ | -------------
[pr_ts_taf(df, 1921, 2007, scaling factor)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/june2018plots/d418_ridges.jpg)|  monthly timeseries stacked on one page - color scale: water year totals (sums)
