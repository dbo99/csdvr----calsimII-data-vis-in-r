...
0. Requires `.csv` output (not `.dss`). Specifying `.csv` instead of `.dss` in WRIMS' dv field produces both `.csv` and `.dss` output decision variable "dv" files. Requires WRIMS versions from 2018 or later. Accepts either CalSimII or CalSim 3.0 output, but set up currently only for CalSimII's time range.
