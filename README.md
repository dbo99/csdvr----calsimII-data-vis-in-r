...
[calsim](https://water.ca.gov/Library/Modeling-and-Analysis/Central-Valley-models-and-tools/CalSim-2) is a water resources planning model for a large part of california, developed jointly by [usbr](https://www.usbr.gov/) and [ca-dwr](https://water.ca.gov/) to help analyze the limitations and possibilities of, and benefits and impacts from, the interconnected [central valley project](https://www.usbr.gov/mp/cvp/) and [state water project](https://water.ca.gov/Programs/State-Water-Project), under alternative infrastructure, regulatory, operational, or climate scenarios.   calsim also helps address related engineering and scientific questions, by its output commonly being used as input to popular biologic, economic, groundwater, and hydrodynamic models
   ...
   key file  | use
------------ | -------------
[`csv_list.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/Aug15/csv_list.r) | list csv file names and assign brief unique scenario nicknames
[`fun_defs.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/Aug15/fun_defs.r) | function definitions - add to here as necessary
[`scen_facts.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/Aug15/scenfacts.r) | list same nicknames top down to order the left to right position of scenarios in the plot (instead of alphabetic)
[`plotexport.r`](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/Aug15/plotexport.r) | select which plot scripts to run for export (wait 1-2 minutes for a comprehensive set or a few seconds for 20 or so?)
...
[p_mon_excd2_taf(df)](https://github.com/dbo99/csdvr----calsimII-data-vis-in-r/blob/1.01/plots/july2018plots/s1%26s4%26s6%26s8%26s11_storagemonthly.jpg) |  monthly exceedance in taf, style 2: plot ("facet") for each dv by column
p_mon_excd3_taf | monthly exceedance in taf, style 2: plot ("facet") for each dv by row
