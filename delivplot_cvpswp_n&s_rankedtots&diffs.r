...
setwd("..")
source("scenfacts.r")
p1<-   pb_mn_ann_perav_taf_nolab_rank(df) + ggtitle("Mean Annual Baseline and Scenario Delivery Volumes")+
...
df_diff <- create_df_diff(df)
source("scenfacts.r")
p2 <- pb_mn_ann_perav_taf_d_nolab_rank(df_diff) + ggtitle("Mean Annual Delivery Volume, Difference from Baseline")+
...
p <- plot_grid(p1, p2, nrow = 2, rel_heights = c(1,2.5))
p <- plot_grid(p, pleg, nrow = 1, rel_widths = c(6,1))
setwd("./plots")
